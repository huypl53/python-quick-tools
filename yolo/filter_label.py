...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--keep-empty",
        action="store_true",
        help="Copy images even if all annotations were removed; writes empty label files.",
    )
    return parser.parse_args()


def load_label_indices(data_yaml: Path, drop_names: Iterable[str]) -> Set[int]:
    ...
    return {name_to_idx[name] for name in drop_names}
...
def filter_label_file(
    label_path: Path, drop_indices: Set[int]
) -> tuple[List[str], int]:
    filtered: List[str] = []
    drop_count = 0
...
def copy_metadata(source: Path, dest: Path) -> None:
    for filename in ["data.yaml", "README.dataset.txt", "README.roboflow.txt"]:
...
def main() -> None:
    ...
    drop_indices = load_label_indices(source / "data.yaml", args.drop)
    ...
    for dirpath, dirnames, filenames in os.walk(source):
        ...
        for filename in filenames:
            ...
            filtered_lines, removed_count = filter_label_file(label_path, drop_indices)
            removed_labels += removed_count
            image_stem = Path(filename).stem
            image_path = find_image_path(images_dir, image_stem)
            if not filtered_lines and not args.keep_empty:
                if not image_path:
                    print(
                        f"Skipped {label_path} (no remaining annotations and image missing)."
                    )
                continue
